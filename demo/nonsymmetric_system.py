...
def main():
    ...
    x_sym_cg, info_sym_cg = amg_solve(A_sym, b)
    print(f"Symmetric matrix (skew=0.0), Solver: CG")
    print(info_sym_cg)

    x_nonsym_cg, info_nonsym_cg = amg_solve(A_nonsym, b)
    print(f"\nNon-symmetric matrix (skew=0.5), Solver: CG")
    print(info_nonsym_cg)

    x_nonsym_bicg, info_nonsym_bicg = amg_solve(
        A_nonsym, b, config={"solver": "BICGSTAB"}
    )
    print(f"\nNon-symmetric matrix (skew=0.5), Solver: BiCGSTAB")
    print(info_nonsym_bicg)

    x_nonsym_op_bicg, info_nonsym_op_bicg = amg_solve(
        A_op_nosym, b, config={"solver": "BICGSTAB"}
    )
    print(f"\nNon-symmetric operator (skew=0.5), Solver: BICGSTAB")
    print(info_nonsym_op_bicg)

    x_nonsym_op_gmres, info_nonsym_op_gmres = amg_solve(
        A_op_nosym, b, config={"solver": "GMRES", "preconditioner": "JACOBI_L1"}
    )
